...
async function runAdvancedRiskProof(_item: RadarItem): Promise<void> {
  ...
  // TODO: Append Builder Code
  await new Promise(r => setTimeout(r, 1500))
}

type ScanState = 'idle' | 'loading' | 'done' | 'error'

function Modal({ item, onClose }: { item: RadarItem; onClose: () => void }) {
      ...
      >
        <button
          ...
          style={{
            ...
            color: '#4a6a80', fontSize: 18, cursor: 'pointer',
        ...
        <div style={{ background: 'rgba(255,184,0,0.05)', border: '1px solid rgba(255,184,0,0.15)', borderRadius: 6, padding: '14px 16px', marginBottom: 20 }}>
          ...
          <div style={{ fontSize: 28, fontWeight: 800, color: '#e8f4ff', fontFamily: 'Syne, sans-serif' }}>
            ~$0.02 <span style={{ fontSize: 13, color: '#4a6a80', fontFamily: 'Space Mono, monospace' }}>USDC on Base</span>
          </div>
        </div>

        <p style={{ fontSize: 10, color: '#2a4a60', fontFamily: 'Space Mono, monospace', marginBottom: 20, lineHeight: 1.7 }}>
          ✓ Non-custodial. We never control your funds.<br />
          ✓ All transactions happen in your wallet.
        </p>

        {proofState === 'loading' && (
          <div style={{ padding: '12px 14px', background: 'rgba(255,184,0,0.04)', border: '1px solid rgba(255,184,0,0.15)', borderRadius: 6, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 12 }}>
            <span style={{ fontSize: 18, animation: 'spin 1s linear infinite', display: 'inline-block', color: '#ffb800' }}>◌</span>
            <span style={{ fontSize: 10, color: '#ffb800', fontFamily: 'Space Mono, monospace', letterSpacing: '0.1em' }}>CONNECTING WALLET...</span>
          </div>
        )}

        {proofState === 'done' && (
          <div style={{ padding: '12px 14px', background: 'rgba(0,229,160,0.04)', border: '1px solid rgba(0,229,160,0.2)', borderRadius: 6, marginBottom: 16 }}>
            <span style={{ fontSize: 10, color: '#00e5a0', fontFamily: 'Space Mono, monospace' }}>✓ VERIFICATION COMPLETE!</span>
          </div>
        )}

        {proofState === 'error' && (
          <div style={{ padding: '12px 14px', background: 'rgba(255,59,92,0.04)', border: '1px solid rgba(255,59,92,0.2)', borderRadius: 6, marginBottom: 16 }}>
            <span style={{ fontSize: 10, color: '#ff3b5c', fontFamily: 'Space Mono, monospace' }}>✗ CONNECTION FAILED — RETRY</span>
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button
            onClick={handleProof}
            disabled={proofState === 'loading' || proofState === 'done'}
            style={{
              flex: 1, padding: '12px', borderRadius: 6, border: 'none',
              background: proofState === 'done' ? '#00e5a0' : proofState === 'loading' ? 'rgba(255,184,0,0.5)' : 'linear-gradient(135deg, #ffb800, #e09000)',
              color: '#020508', fontWeight: 800,
              cursor: proofState === 'loading' ? 'wait' : 'pointer',
              fontSize: 12, fontFamily: 'Space Mono, monospace', letterSpacing: '0.08em',
              transition: 'all 0.2s',
            }}
          >
            {proofState === 'loading' ? '◌ CONNECTING...' : proofState === 'done' ? '✓ VERIFIED!' : 'CONNECT & VERIFY →'}
          </button>
          <button
            onClick={onClose}
            style={{ padding: '12px 16px', borderRadius: 6, border: '1px solid rgba(0,200,255,0.1)', background: 'transparent', color: '#4a6a80', cursor: 'pointer', fontSize: 12, fontFamily: 'Space Mono, monospace' }}
          >
            CANCEL
          </button>
        </div>
...
export default function RadarCard({ item }: { item: RadarItem }) {
  ...
  const btn = (state: ScanState, color: string, idle: string, loading: string, done: string) => ({
    style: {
      ...
      fontSize: 10, fontWeight: 700, cursor: state === 'loading' ? 'wait' as const : 'pointer' as const,
      fontFamily: 'Space Mono, monospace', letterSpacing: '0.06em',
      transition: 'all 0.15s', whiteSpace: 'nowrap' as const,
    },
    label: state === 'loading' ? loading : state === 'done' ? `✓ ${done}` : state === 'error' ? '✗ RETRY' : idle,
  })

  const analyzeBtn = btn(analyzeState, '#00c8ff', '👀 ANALYZE', '⟳ ANALYZING...', 'ANALYZED')
  const scanBtn = btn(deepScanState, '#a5b4fc', '🛡️ DEEP SCAN', '◌ SCANNING...', 'SCANNED')
      ...
      <div style={{ background: 'rgba(6,13,20,0.98)', border: '1px solid rgba(0,200,255,0.07)', borderLeft: `3px solid ${riskColor}`, borderRadius: 6, overflow: 'hidden' }}>
        <div style={{ padding: '14px 18px' }}>
          ...
          {analyzeState === 'loading' && (
            <div style={{ marginBottom: 10, padding: '10px 14px', background: 'rgba(0,200,255,0.04)', border: '1px solid rgba(0,200,255,0.12)', borderRadius: 4, display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ display: 'flex', gap: 3, alignItems: 'flex-end', height: 16 }}>
                {[0,1,2,3,4].map(i => <div key={i} style={{ width: 3, background: '#00c8ff', borderRadius: 2, animation: 'barPulse 0.7s ease infinite alternate', animationDelay: `${i*0.1}s`, height: `${8 + i*2}px` }} />)}
              </div>
          ...
          {analyzeState === 'done' && analysisResult && (
            <div style={{ marginBottom: 10, padding: '10px 14px', background: 'rgba(0,200,255,0.04)', border: '1px solid rgba(0,200,255,0.15)', borderRadius: 4 }}>
              <div style={{ fontSize: 8, color: '#00c8ff88', fontFamily: 'Space Mono, monospace', letterSpacing: '0.15em', marginBottom: 4 }}>ANALYSIS RESULT</div>
              <div style={{ fontSize: 11, color: '#7ab8d0', fontFamily: 'Space Mono, monospace', lineHeight: 1.6 }}>🔍 {analysisResult}</div>
            </div>
          )}
          {analyzeState === 'error' && (
            <div style={{ marginBottom: 10, padding: '10px 14px', background: 'rgba(255,59,92,0.04)', border: '1px solid rgba(255,59,92,0.2)', borderRadius: 4 }}>
              <span style={{ fontSize: 10, color: '#ff3b5c', fontFamily: 'Space Mono, monospace' }}>✗ ANALYSIS FAILED — RETRY</span>
            </div>
          )}
          {deepScanState === 'loading' && (
            <div style={{ marginBottom: 10, padding: '10px 14px', background: 'rgba(99,102,241,0.04)', border: '1px solid rgba(99,102,241,0.15)', borderRadius: 4, display: 'flex', alignItems: 'center', gap: 12 }}>
              <span style={{ fontSize: 16, animation: 'spin 1s linear infinite', display: 'inline-block', color: '#a5b4fc' }}>◌</span>
          ...
          {deepScanState === 'done' && deepScanResult && (
            <div style={{ marginBottom: 10, padding: '10px 14px', background: deepScanResult.includes('⚠') ? 'rgba(255,184,0,0.04)' : 'rgba(0,229,160,0.04)', border: `1px solid ${deepScanResult.includes('⚠') ? 'rgba(255,184,0,0.2)' : 'rgba(0,229,160,0.2)'}`, borderRadius: 4 }}>
              <div style={{ fontSize: 8, color: '#a5b4fc88', fontFamily: 'Space Mono, monospace', letterSpacing: '0.15em', marginBottom: 4 }}>DEEP SCAN RESULT</div>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
            <button onClick={handleAnalyze} disabled={analyzeState === 'loading'} style={analyzeBtn.style}>{analyzeBtn.label}</button>
            <button onClick={handleDeepScan} disabled={deepScanState === 'loading'} style={scanBtn.style}>{scanBtn.label}</button>
            <TrackButton pairId={item.id} pair={item.pair} />
            <AlertButton pairId={item.id} pair={item.pair} />
            <button
              onClick={() => setShowModal(true)}
              style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '6px 12px', borderRadius: 4, border: '1px solid rgba(255,184,0,0.3)', background: 'rgba(255,184,0,0.06)', color: '#ffb800', fontSize: 10, fontWeight: 700, cursor: 'pointer', fontFamily: 'Space Mono, monospace', letterSpacing: '0.06em', marginLeft: 'auto', whiteSpace: 'nowrap' }}
            >
              🛡️ ADV. RISK PROOF
            </button>
